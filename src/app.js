...
const stateUI={route:location.pathname,returnRoute:'/',tab:'world',selected:null,detailOpen:false,modal:null,toast:'',filter:'all',prologue:0,map:{x:0,y:0,w:900,h:680},nameSuggestion:'长夜',activeOutbreak:null,dropChoice:null,logOpen:null};
...
function diseasePage(){return pageFrame('择一疫，降于人间。','你是「疫」本身。疫病是你落在人间的四种形状。',`<div class="disease-grid">${diseases.map((d,i)=>`<article class="disease-card disease-${i}"><span class="eyebrow">疫册 / ${String(i+1).padStart(2,'0')}</span><div class="disease-glyph">${['寒','疽','水','疮'][i]}</div><h2>${d.name}</h2><p class="verse">${d.line}</p><p>${d.desc}</p><div class="tag-row">${d.tags.map(t=>pill(t)).join('')}</div><button class="primary" data-action="first-disease" data-id="${d.id}">降此疫 →</button></article>`).join('')}</div>`,'/prologue');}
...
function hud(){return `<div class="hud"><div class="hud-date"><span class="eyebrow">${esc(game.name)}之疫 · ${stageName(game.scar)}</span><strong>${periodName(game.turn)}</strong></div><div class="hud-stats"><div><span>疫势</span><strong>${game.power}</strong></div><div><span>疫痕</span><strong>${game.scar}<small> / ${[20,45,75,110].find(x=>x>game.scar)||'终'}</small></strong></div><div class="alert"><span>朝警 · ${alertName(game.alert)}</span><strong>${game.alert}<small> / 100</small></strong></div></div><button class="plain hud-menu" data-action="pause" aria-label="暂停与设置">☰</button></div>`;}
...
function overlay(){if(!stateUI.modal)return '';
 if(stateUI.modal==='report'){const r=game.lastReport;return `<div class="overlay"><div class="modal report" role="dialog" aria-modal="true" aria-label="旬结算"><span class="eyebrow">旬报 / ${periodName(r.turn)}</span><h2>这一旬，天下又变了。</h2><div class="report-section"><h3>本旬大事</h3>${r.headlines.map(t=>`<p>· ${esc(t)}</p>`).join('')}</div><div class="report-stats"><div><span>新染疫地区</span><strong>${r.newRegions}</strong></div><div><span>疫势</span><strong>${r.power>=0?'+':''}${r.power}</strong></div><div><span>疫痕</span><strong>+${r.scar}</strong></div><div><span>朝警</span><strong>${r.alert>=0?'+':''}${r.alert}</strong></div></div><div class="report-section faction-report"><h3>王朝动向</h3>${r.factions.map(t=>`<p>· ${esc(t)}</p>`).join('')}</div>${button('close-modal','继续观天下 →','primary wide')}</div></div>`;}
 ...
 if(stateUI.modal==='pause')return `<div class="overlay"><div class="modal compact" role="dialog" aria-modal="true"><span class="eyebrow">暂停 / 卷宗</span><h2>${esc(game.name)}之疫</h2><p>${periodName(game.turn)} · 自动存档中</p>${button('close-modal','继续观天下','primary wide')}${button('settings','设置','secondary wide')}${button('archive','档案','secondary wide')}${button('home','返回卷首','ghost wide')}</div></div>`;

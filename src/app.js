...
const mapBoxFor=(mode='realm',macroId=null)=>{
 if(mode==='realm')return window.innerWidth<700?{x:60,y:-65,w:880,h:810}:{x:-100,y:-65,w:1180,h:810};
...
function regionMapArt(interactive=true){
 ...
 const line=(edges,cls)=>edges.filter(([a,b])=>members.includes(a)&&members.includes(b)).map(([a,b])=>{const p=one(regions,a),q=one(regions,b),d=`M${p.x} ${p.y} Q${(p.x+q.x)/2+8} ${(p.y+q.y)/2-6} ${q.x} ${q.y}`;const casing=cls==='water'?'':`<path class="road-casing" d="${d}"/>`;return `${casing}<path class="${cls}" d="${d}"/>`;}).join('');
 // 军路：与军镇相连的官道（方案 5.3）

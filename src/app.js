...
const ICON_DEFS={
 palace:'<path d="M-14 13V2h28v11M-11 2L0-6l11 8M-7-6.5L0-12l7 5.5M0-12v-4.5M-8 13V6M8 13V6M-2.5 13V8h5v5"/>',
 cityGate:'<path d="M-10 13V2h20v11M-8 2L0-5l8 7M0-5v-3M-3.5 13V8h7v5"/>',
 watchtower:'<path d="M-7 13L-5 2h10l2 11M-2.5 13V8h5v5M0 2v-10.5M0-8.5h7L4.5-5.5 0-4v1.5"/>',
 harbor:'<path d="M-2 6V-11M-2-9.5c-5.5 1.5-7.5 6-7 10.5h7M-2-8.5c4.5 1.2 6 5 5.5 8H-2M-11.5 9.5q3.5-3.2 7 0t7 0 7 0" class="ic-hull"/>',
 granary:'<path d="M0 2C3 1 5.5-3 5.5-8H-5.5C-5.5-3-3 1 0 2ZM-10.5 13V2h21v11M-6 13c0-4.5 2.5-7 6-7s6 2.5 6 7"/>',
 frontierFort:'<path d="M-12 13v-7l3-3 3 3v7M7 13V0h5v13M0 13h12M12 0v-9M12-7h6l-3 2.5 3 2.5h-6"/>',
 grandGranary:'<path d="M-4 2C-1 1 .5-2 .5-7h-9C-8.5-2-7 1-4 2ZM-13 13V2h9v11M4 13V2h9v11M-9.5 13c0-4 2-6 4.5-6s4.5 2 4.5 6M8.5 13c0-4 2-6 4.5-6s4.5 2 4.5 6"/>',
 bridgeCity:'<path d="M-10 2V-4h20V2M-8-4l-3-3.5M8-4l3-3.5M-14 8.5Q-7 2.5 0 8.5t14 0M-10 8.5V13M0 8.5V13M10 8.5V13M-14 13h28" class="ic-bridge"/>',
 ferryPort:'<path d="M-10.5 4c3.5-3.2 7-3.2 9.5-.5L2 0M1-1l1.5 5M-13 9.5h7l2.5 3h-10ZM-2.5 9.5l2-4.5" class="ic-ferry"/>',
 riverPort:'<path d="M-2 5V-12M-2-10.5c-6 1.5-8 6.5-7.5 11.5h7.5M-2-9c5 1.5 6.5 5.5 6 9H-2M-13 9q4-3.5 8 0t8 0 8 0"/>',
 remove:null
};
...
const ICON_SCALE={capital:2.05,granary:1.7,military:1.7,port:1.65,prefecture:1.5};
...
const LAND_PATH='M95 88 Q153 35 250 58 L380 32 L515 44 Q636 30 780 90 L830 158 L836 301 Q817 391 801 466 L722 604 L610 640 L514 628 L415 639 L289 588 L157 553 L90 440 L55 306 Z';
const MACRO_PATHS={
...
function macroTerrainArt(id){
 if(id==='north')return '<path d="M145 130l24-27 24 27 25-34 28 34M625 126l22-24 22 24"/>';
 if(id==='hedong')return '<path d="M125 290q90-35 205 0M110 324q115-35 245 0M120 358q110-30 220 2"/>';
 if(id==='linjin')return '<path d="M510 235q70 35 160 3t120 13M485 300q85 28 175-8t130 8"/>';
 if(id==='luonan')return '<ellipse cx="430" cy="520" rx="115" ry="42"/><ellipse cx="505" cy="568" rx="92" ry="31"/>';
 if(id==='donghai')return '<path d="M733 425q35 35 8 78t6 71M770 424q30 30 9 58"/>';
 return '<path d="M455 92v58M425 121h62M434 102l21-13 21 13"/>';
}
function realmMapArt(interactive=true){
 ...
 const area=m=>{const stats=macroRegionStats(current,m.id),eventList=macroRegionEvents(current,m.id),selected=stateUI.selected===m.id;
  ...
  return `<g class="macro-region severity-${stats.severity} ${selected?'selected':''}" ${interactive?`data-macro="${m.id}" tabindex="0" role="button" aria-label="查看${m.name}，${severityName[stats.severity]}"`:''}><path class="macro-shape" d="${MACRO_PATHS[m.id]}"/><g class="macro-terrain macro-terrain-${m.id}">${macroTerrainArt(m.id)}</g><g clip-path="url(#clip-${m.id})">${stats.severity?`<path class="macro-infection-wash" d="${MACRO_PATHS[m.id]}"/>`:''}${auras}</g></g>`;};
 const title=m=>{const stats=macroRegionStats(current,m.id),eventList=macroRegionEvents(current,m.id),diseaseText=stats.activeDiseases.slice(0,2).map(d=>d.glyph).join('·')+(stats.activeDiseases.length>2?` +${stats.activeDiseases.length-2}`:'');return `<g class="macro-title" transform="translate(${m.x} ${m.y})"><text>${m.name}</text><text class="macro-status" y="24">${severityName[stats.severity]}${stats.infected?` · ${number(stats.infected)}`:''}${diseaseText?` · ${diseaseText}`:''}</text>${eventList.length?`<text class="macro-event" x="48" y="-17">事 ${eventList.length}</text>`:''}</g>`;};
 return `<svg class="world-svg realm-svg" viewBox="${box.x} ${box.y} ${box.w} ${box.h}" role="img" aria-label="靖朝六大区域天下舆图"><defs><filter id="macro-glow"><feGaussianBlur stdDeviation="18"/></filter><path id="mn" d="M-4 4 Q0 -4 4 4" fill="none"/>${macroRegions.map(m=>`<clipPath id="clip-${m.id}"><path d="${MACRO_PATHS[m.id]}"/></clipPath>`).join('')}</defs><path class="land-shadow" d="M90 85 Q155 18 250 48 L380 20 L515 30 Q635 14 790 78 L842 152 L848 305 Q830 392 815 475 L728 615 L614 650 L514 637 L415 651 L285 600 L149 564 L79 448 L44 304 Z"/>${ordered.map(area).join('')}<g class="macro-routes">${MAIN_LINKS.map(link).join('')}</g>${ordered.map(title).join('')}</svg>`;
}
...
function regionMapArt(interactive=true){
 ...
 const node=r=>{const outbreaks=game?regionOutbreaks(game,r.id):[],selected=stateUI.selected===r.id,eventList=game?activeEvents(game,r.id):[],status=statusMark(eventList);
   ...
   return `<g class="map-node ${selected?'selected':''} ${outbreaks.length?'infected':''} ${surged?'surging':''} ${allDormant?'dormant':''} ${r.type} tier-${tier}" ${interactive?`data-region="${r.id}" tabindex="0" role="button" aria-label="查看${r.name}${outbreaks.length?`，已有${plagueNames}，病者估计${number(totalInfected)}`:''}"`:''} transform="translate(${r.x} ${r.y})">${outbreaks.length?`<circle class="infection-aura" r="${auraRadius.toFixed(1)}"/>`:''}${surged?`<circle class="surge-ring" r="${(auraRadius+5).toFixed(1)}"/>`:''}<circle class="hit" r="30"/><circle class="node-ring" r="11"/><g class="landmark" transform="scale(${scale.toFixed(2)})">${iconPath}</g><text class="node-name" y="-${(13*scale+6).toFixed(0)}">${r.name}</text>${status?`<g class="status-mark ${status.cls}" transform="translate(${(13*scale).toFixed(0)} -${(13*scale).toFixed(0)})" aria-label="${esc(status.title)}"><circle r="6.5"/><text y="3.4">${status.glyph}</text></g>`:''}${outbreaks.length?`<text class="sick-count disease-marks" y="${markY+13}">${plagueText}</text>`:''}</g>`;};
 return `<svg class="world-svg region-svg" viewBox="${box.x} ${box.y} ${box.w} ${box.h}" role="img" aria-label="${macro?.name||''}区域舆图"><defs><pattern id="grain" width="8" height="8" patternUnits="userSpaceOnUse"><circle cx="1" cy="2" r=".4" fill="#a99275" opacity=".25"/></pattern><radialGradient id="infection-aura-gradient" cx="50%" cy="50%" r="50%"><stop offset="0%" stop-color="#c7443b" stop-opacity=".3"/><stop offset="52%" stop-color="#b83f38" stop-opacity=".17"/><stop offset="78%" stop-color="#9d342f" stop-opacity=".07"/><stop offset="100%" stop-color="#8d2f2b" stop-opacity="0"/></radialGradient><path id="mn" d="M-4 4 Q0 -4 4 4" fill="none"/></defs><path class="land-shadow" d="M90 85 Q155 18 250 48 L380 20 L515 30 Q635 14 790 78 L842 152 L848 305 Q830 392 815 475 L728 615 L614 650 L514 637 L415 651 L285 600 L149 564 L79 448 L44 304 Z"/><path class="land" d="${LAND_PATH}"/><path class="land-grain" d="${LAND_PATH}"/><path class="region-focus" d="${MACRO_PATHS[macro?.id]||LAND_PATH}"/><g class="terrain"><g class="terrain-north"><path class="ridge" d="M85 118l28-32 28 30 26-40 30 42 24-30 30 34M126 218l24-28 26 26 22-34 26 36"/><path class="ridge far" d="M60 160l20-24 20 22 18-30 24 30"/><path class="wind" d="M92 268q26-10 48 2M132 292q20-8 40 1"/></g><g class="terrain-east-plain"><path class="field" d="M150 352h64M150 366h64M158 380h50M235 418h62M237 432h58"/><path class="dry" d="M200 470l14 9 14-9"/></g><g class="terrain-river"><path class="riverline" d="M352 478q48 20 96 6t104 30M360 500q40 16 80 6M470 530q44 10 88-4"/></g><g class="terrain-lakes"><path class="mist" d="M470 480q30-8 60-2M508 512q26-7 52-1M448 540q24-6 48 0"/><path class="pool" d="M540 470q16-6 30 1 12 6-3 11t-27-4q-9-5 0-8ZM470 558q13-5 25 0 10 5-2 9t-23-3q-8-4 0-6Z"/></g><g class="terrain-coast"><path class="coastline" d="M700 180q35 20 55 60t12 95M724 412q22 42 18 92"/><path class="wave" d="M768 250q6-4 12 0t12 0M760 296q6-4 12 0t12 0M768 470q6-4 12 0t12 0M742 536q6-4 12 0t12 0"/><path class="sea-route" d="M610 525q90 30 180-80M735 440q60-30 55-118"/></g></g>${line(civilRoads,'road')}${line(military,'road military-road')}${line(waterways,'water')}${actors.map(a=>a.flag?`<g class="actor ${a.cls}"><path d="M0 0V-9M0-9h7l-2.4 2.6L7-4H0"/><animateMotion dur="${a.dur}" repeatCount="indefinite" path="${a.path}"/></g>`:`<circle class="actor ${a.cls}" r="${a.r}"><animateMotion dur="${a.dur}" begin="${a.delay}" repeatCount="indefinite" path="${a.path}"/></circle>`).join('')}${visible.map(node).join('')}</svg>`;
}
...
app.addEventListener('click',e=>{
 ...
 else if(action==='first-disease'){if(!game)game=newGame();game.firstDisease=id;saveGame(game);stateUI.dropChoice=id;stateUI.tab='world';stateUI.mapMode='realm';stateUI.activeMacroRegion=null;stateUI.selected=null;stateUI.map=defaultMap();go('/game');}
...
window.addEventListener('keydown',e=>{if(e.key==='Escape'){if(stateUI.modal)stateUI.modal=null;else if(stateUI.detailOpen)stateUI.detailOpen=false;else if(stateUI.mapMode==='region'){stateUI.mapMode='realm';stateUI.activeMacroRegion=null;stateUI.selected=null;stateUI.map=defaultMap();}render();}});
render();
